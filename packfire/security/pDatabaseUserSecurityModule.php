...
/**
 * pDatabaseUserSecurityModule Description
 *
...
class pDatabaseUserSecurityModule implements ISecurityModule {
    ...
    public function identity($newIdentity = null) {
        ...
        if(func_num_args() == 1){
            if($newIdentity){
                $user = $this->service('database')->from('users')->get(array('Username' => $newIdentity));
                $this->service('session')->set('security.user.id', $user['UserId']);
            }else{
            ...
            if($id !== null){
                $user = $this->service('database')->from('users')->get(array('UserId' => $id));
            }
        }
        if($user){
            $this->user = $user;
